...
public class GestorGame {
	...
	public void startGame(int humans, int ais) {
		...
		for (int i = 0; i < humans; i++) players.add(new BuilderPlayer().setName("H"+(i+1)).setIsAi(false).build());
		for (int i = 0; i < ais; i++) players.add(new BuilderPlayer().setName("IA"+(i+1)).setIsAi(true).build());
